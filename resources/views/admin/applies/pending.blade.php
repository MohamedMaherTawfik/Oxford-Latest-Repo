<x-panel>
    ...
    <div class="p-6 bg-white min-h-screen">
        ...
        <div class="hidden md:grid grid-cols-7 gap-4 font-semibold text-gray-700 mb-2">
            <div class="text-[#79131d] truncate">{{ __('main.user') }}</div>
            <div class="text-[#79131d] truncate">{{ __('main.email') }}</div>
            <div class="text-[#79131d] truncate">{{ __('main.phone') }}</div>
            <div class="text-[#79131d] truncate">{{ __('main.cv') }}</div>
            <div class="text-[#79131d] truncate">{{ __('main.certificate') }}</div>
        ...
        @foreach ($applies as $apply)
            <div x-data="{ openRejectModal: false, openMailModal: false }"
                ...
                <div class="flex items-center gap-2 overflow-hidden">
                    <div class="w-10 h-10 flex-shrink-0 bg-gray-300 rounded-full">
                ...
                {{-- Email --}}
                <div class="text-gray-700 truncate">{{ $apply->user->email }}</div>

                {{-- Phone --}}
                <div class="text-gray-700 truncate">{{ $apply->phone }}</div>
                ...
                {{-- Actions --}}
                <div class="flex justify-end items-center space-x-2 text-gray-600 text-sm md:text-lg">
                    <form action="{{ route('admin.applies.accept', $apply->id) }}" method="GET">
                        @csrf
                        <button type="submit"
                            class="bg-green-500 hover:bg-green-800 text-white rounded-lg px-3 py-2 whitespace-nowrap">
                            {{ __('main.accept') }}
                        </button>
                    </form>

                    <button type="button" @click="openRejectModal = true"
                        class="bg-red-500 hover:bg-red-800 text-white rounded-lg px-3 py-2 whitespace-nowrap">
                        {{ __('main.reject') }}
                    </button>
                </div>
            </div>
